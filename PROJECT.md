...
- ⏳ `auth.config.ts:82-92` callback "update" cambia `token.tenantId` al `session.tenantId` del cliente sin verificar ownership → potencial IDOR. FIX: añadir `prisma.workspaceUser.findFirst({where:{userId:token.uid,workspaceId:session.tenantId}})` antes de aceptar el switch.
